...
$new = $text -replace "(?m)^external_media_address\s*=.*$", "external_media_address = $Ip"
if ($new -eq $text) { throw "external_media_address line not found in pjsip.conf" }
Set-Content -LiteralPath $conf -Value $new -NoNewline
Write-Output "media address -> $Ip"

$compose = Join-Path (Join-Path $PSScriptRoot "..") ".."
$override = "C:\Users\priya\AppData\Local\Temp\opencode\compose.ports.yml"
docker compose -f (Join-Path $compose "docker-compose.yml") -f $override up -d --force-recreate asterisk | Select-Object -Last 1

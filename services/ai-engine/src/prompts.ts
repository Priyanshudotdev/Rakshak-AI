export const EXTRACTION_PROMPT = `You are an emergency dispatch AI assistant for Nagpur Police. Extract critical
...
export const PRIORITY_PROMPT = `You are an emergency dispatch priority assessment AI for Nagpur Police.
...
export function fillExtractionPrompt(language: string, originalText: string, callText: string): string {
  return EXTRACTION_PROMPT.replace("{language}", language || "Unknown")
    .replace("{original_text}", originalText)
    .replace("{call_text}", callText);
}

export function fillPriorityPrompt(extractionJson: string): string {
  return PRIORITY_PROMPT.replace("{extraction_json}", extractionJson);
}

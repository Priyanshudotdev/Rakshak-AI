...
16. **Embeddings + geo/semantic search live (2026-09-16):** decision — Gemini `gemini-embedding-001` with `outputDimensionality: 1536`, exactly `VECTOR(1536)` (`services/ai-engine/src/embeddings.ts`, null-safe). Sync backfills vectors best-effort; `GET /api/incidents/nearby` (PostGIS `ST_DWithin`, distance-sorted) and `POST /api/incidents/similar` (pgvector cosine → score) proven on Neon: nearby 21.14,79.07 → 2 hits @641 m; similar "shop on fire…" → Fire 0.744 > Traffic 0.554. Note: sync can hit a Neon cold-start timeout — retry succeeds.

...
17. **Gateway ARI client live (2026-09-16):** hand-rolled ARI over `ws`+fetch (`services/media-gateway/src/ari.ts`, no extra dep): answers `Stasis(rakshak)` calls, forks per-call slin16 External Media (one UDP port per call, no SSRC demux), bridges caller+media, streams RTP payloads into the realtime adapter, tears down on hangup. 7 unit tests (fake socket/fetch) + build green; `REALTIME=saaras` + ARI env wired in compose; gateway logs `ari connected`, `/health` reports `realtime`. Dial `9000` from a Linphone for the full call→transcript→incident loop. TTS-audio *injection* back into the call is the explicit next step (RTP send path).
18. **Two-way translated conversation (2026-09-17):** operator dials `9002` → own STT fork + joins the live emergency bridge (role-aware teardown). Caller finals: incident + reply in caller tongue, plus auto-translation played to the operator (`OPERATOR_LANG`, default en-IN). Operator finals: translated toward the caller tongue, played to the caller — operator speech never creates incidents. Dashboard Operator-listen card = manual per-utterance translated playback (browser-private). Expect ~5–10 s turn latency (STT final + MT + TTS); bridge partners may also hear channel-targeted playbacks (accepted v1 redundancy).
...
15. **Operator auth (2026-09-16):** `005_operator_auth.sql` (bcrypt hashes, `operator_sessions` with 24h expiry, first-registered is admin); routes `POST /api/operators/register|login|logout`, `GET /api/operators/me`, `POST /api/operators/change-password` (verifies current, revokes other sessions); actor order Bearer > `x-operator` > `operator`; mutating routes hard-require Bearer only when `AUTH_REQUIRED=1` (worker sends `OPERATOR_TOKEN` when set); dashboard Dispatch panel signs in/out and sends the token. Parity-tested both backends; hashes never leave the server. Migration notes: `operator_id` is TEXT without FK (001-UUID vs fresh-TEXT stores), `expires_at` app-supplied — pg-mem constraints documented inline. Live: `priya`/admin registered on Neon.
16. **Embeddings + geo/semantic search live (2026-09-16):** decision — Gemini `gemini-embedding-001` with `outputDimensionality: 1536`, exactly `VECTOR(1536)` (`services/ai-engine/src/embeddings.ts`, null-safe). Sync backfills vectors best-effort; `GET /api/incidents/nearby` (PostGIS `ST_DWithin`, distance-sorted) and `POST /api/incidents/similar` (pgvector cosine → score) proven on Neon: nearby 21.14,79.07 → 2 hits @641 m; similar "shop on fire…" → Fire 0.744 > Traffic 0.554. Compose postgres stays record-store-only (pgvector image has no PostGIS) — both routes answer a clear 400 there instead of a 500. Note: sync can hit a Neon cold-start timeout — retry succeeds.
